...
"""
price_category filter
"""
def getRecomendationPriceCategory(cursor, profileID):
    print("Hoi")
    prijsRange = [["0-1",0,1],["2-4",2,4],["5-10",5,10],["11-20",11,20],["21-30",21,30],["31-40",31,40],["41-50",41,50],
                  ["51-75",51,75],["76-100",76,100],["101-150",101,150],["151-200",151,200],["201-250",201,250],
                  ["250-325",250,325],["325+",325,99999]]
    prijzen = {}
    profileData = get_profiles_content2(cursor, profileID, ["products.id", "orders.aantal", "products.price",
                                                            "orders.sessions_id_key", "sessions.profiles_id_key"],
                                        ["products", "main_category", "orders", "sessions"],
                                        ["products.main_category_id_key", "main_category.id", "orders.products_id_key",
                                         "products.id","orders.sessions_id_key", "sessions.id", "profiles_id_key", new_item])
    ...
    #     if item not in prijzen:


def recomendationPriceCategory(db, cursor):
    print("Hoi")
    profiles = retrieve_order_profiles(cursor)

    for profile in profiles:
        profileContent, recomendationContent, priceCategory = getRecomendationPriceCategory(cursor,profile[0])
        print(priceCategory)